...
#used echo command to print the title for Task 1
echo "SetGid files:"
echo "============="
# find command modified as needed
# used find command to run on root folder and find files that ahve 2000 perm
#used sort to sort the files 
find / -type f -executable -perm -4000 -ls 2>/dev/null | sort -k 3
# sort command to sort files appropriately

# commands to display a third title
echo "ten biggest files:"
echo "=================="
# commands to make a list of the 10 biggest files
find / -type f -exec ls -al --block-size=M {} + 2>/dev/null | sort -nr -k 5 | awk '{ print $3" | "$5" | "$9}' | head -10
