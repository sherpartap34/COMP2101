...
# divided the commands and completed task 1
host=$(hostname)
LanAddressData=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
LanAddressOutput=$(ip a s $LanAddressData |awk '/inet /{gsub(/\/.*/,"");print $2}')
LanHostnameOutput=$(getent hosts $LanAddressOutput| awk '{print $2}')
ExternalIPOutput=$(curl -s icanhazip.com)
ExternalNameOutput=$(getent hosts $ExternalIPOutput | awk '{print $2}')
# Task 2: added default router name to /etc/hosts with default router ip
RouterIPOutput=$(ip route show | grep default | awk '{ print $3}')
RouterNameOutput=$(getent hosts $RouterIPOutput | awk '{print $2}')

cat <<EOF
Hostname        : $host
LAN Address     : $LanAddressOutput
LAN Hostname    : $LanHostnameOutput
External IP     : $ExternalIPOutput
External Name   : $ExternalNameOutput
Router Address  : $RouterIPOutput
Router HostName : $RouterNameOutput
EOF
